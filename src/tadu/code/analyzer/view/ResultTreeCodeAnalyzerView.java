	...
	protected void createContents() {
		...
		startBtn.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				coreSvnUrl = "http://svn.kaiqi.com/tadu/tech/server/tadu-server/dubbo-core/trunk/dubbo-core";
				...
				final java.util.List<ChangedResource> coreChangedResources = SvnAnalyzerTools.analyzeSvnDiffResult(
						coreProjectPath, coreSvnUrl, 86135, 85975);
				...
				java.util.List<ChangedResource> webChangedResources = SvnAnalyzerTools.analyzeSvnDiffResult(
						webProjectpath, webSvnUrl, 86096, 85897);
				...
				for (ChangedResource changedResource : webChangedResources) {
					if (changedResource instanceof ChangedJavaResource) {
						...
						for (String methodName : changedJavaResource.getMethodNames()) {
							...
							if (cmEntity.getMethod() != null && cmEntity.getCalledMethodEntities() != null) {
						...
						if (staticResourcePath.indexOf(".shtml") > 0) {
							List<IPath> shtmllInServlets = JDTUtils.doFileSearch(TaduJavaProject.WEB_IPROJECT,
									pathWithPreDir, "*.java");
							...
							staticResourceMappers.add(new StaticResourceMapper(pathWithPreDir, JDTUtils.doFileSearch(
									TaduJavaProject.WEB_IPROJECT, pathWithPreDir, "*.shtml")));
						}
					}
				}
					...
					for (SearchModel searchModel : returnObj.javaResultsOfinvokedCoreMethod) {
						for (SearchMatch searchMatch : searchModel.searchResults) {
							if (searchMatch.getResource().getFullPath().toString().indexOf("Action") > 0) {
								...
								ResultBean rBean = resultBeans.get(servletName);
								ResultBean searchMethodResultBean = new ResultBean(searchModel.changedOuterMethod
										.getElementName(), null);
								ResultBean invokeResultBean = new ResultBean(sm.getElementName(),
										asList(searchMethodResultBean));
			...
			/**
			 * 启动一个线程去完成core包的方法查找，过滤外包方法，及其在web项目中调用情况。 返回一个future对象
			 * 
			...
			private Future<CoreThreadReturnObj> startCoreThread(final List<ChangedResource> coreChangedResources) {
				...
				Future<CoreThreadReturnObj> future = threadPool.submit(new Callable<CoreThreadReturnObj>() {
					...
					public CoreThreadReturnObj call() throws Exception {
						CallHierarchyGenerator coreCallGenerator = new CallHierarchyGenerator(
								TaduJavaProject.CORE_JAVAPROJECT);
						ArrayList<CalledMethodEntity> cmEntities = new ArrayList<CalledMethodEntity>();
						...
						for (IMethod changedOuterMethod : changedOuterMethodsInCore) {
							...
							result.add(new SearchModel(changedOuterMethod, distinctList(JDTUtils.doJavaSearch(
									TaduJavaProject.WEB_JAVAPROJECT, methodName))));
						}
			...
			class SearchModel {
				...
				public SearchModel(IMethod changedOuterMethod, List<SearchMatch> searchResults) {
					super();
			...
			/**
			 * 该方法会放回静态资源路径的 文件名及上一级目录名 因为上一级目录名通常包含版本号
			 * 
